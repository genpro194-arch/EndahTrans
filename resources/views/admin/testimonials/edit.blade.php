...
@section('page-subtitle', 'Perbarui ulasan pelanggan')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50">
            <h3 class="font-bold text-gray-800">Edit Testimoni &mdash; {{ $testimonial->name }}</h3>
        </div>
        <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label class="cursor-pointer">
                        ...
                        <i class="fas fa-star text-2xl transition star-rating
                                  {{ old('rating', $testimonial->rating) >= $i ? 'text-amber-400' : 'text-gray-200' }}"
                           data-value="{{ $i }}"></i>
                    </label>
                    @endfor
                    <span class="text-sm text-gray-500 ml-2" id="rating-label">{{ old('rating', $testimonial->rating) }} bintang</span>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <div id="img-preview"
                         class="w-16 h-16 rounded-2xl flex-shrink-0 overflow-hidden border-2 border-gray-100">
                        @if($testimonial->image)
                            <img src="{{ asset('storage/' . $testimonial->image) }}" class="w-full h-full object-cover" id="preview-img">
                        @else
                            <div class="w-full h-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-bold text-xl">
                                {{ strtoupper(substr($testimonial->name, 0, 1)) }}
                            </div>
                        @endif
                    </div>
                    <label class="flex-1 cursor-pointer flex flex-col items-center justify-center py-4 border-2 border-dashed border-gray-200 rounded-2xl hover:border-primary-300 hover:bg-primary-50/30 transition">
                        <i class="fas fa-cloud-upload-alt text-xl text-gray-300 mb-1"></i>
                        <span class="text-sm text-gray-500">Ganti foto (opsional)</span>
